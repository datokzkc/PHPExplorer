...
if($page_no < $max_page){
    $next_page = $page_no + 1;
    echo "<a href=\"./".basename(__FILE__)."?rawno=".$page_long."&page=".$next_page."&shuffle=".$is_shuffle."&dirimg=".$dir_img."&tag=".$nowtag."\"> NEXT(".$next_page."ページ) &gt; </a>  <br>\n";
}
...
foreach($disp_list as $key => $folder){
    ...
    if(is_dir($folder)){
        ...
        if($dir_img == 0){
            $key ++;
            echo "<a href = \"./imageshow.php?path={$link}\">".$key.": {$disp_link}</a>(画像一覧表示へ移動)";
            $key --;
        }else{
            ...
            $key --;
                ...
                echo "<br><a href = \"./imageshow.php?path={$link}\">".$key.": {$disp_link}</a>";
                $key --;
            }
        }
        ...
        $key ++;
        echo "<a href = \"./mediaplay.php?path={$link}\">".$key.": {$disp_link}</a>（メディア再生ページへ）";
        $key --;
    }else{
        //画像以外のファイル
        $key ++;
