...
echo "<a href = \"./imageshow.php?redirect=0&path=".rawurlencode($path)."\"> 現在のディレクトリ内の画像一覧</a><br>\n";
...
echo "<p><a href=\"./".basename(__FILE__)."?rawno=".$change_row."&page=".$page_no."&shuffle=".$is_shuffle."&img=".$on_img."&dirimg=".$dir_img."&path=".rawurlencode($path)."\">"."１ページ".$change_row."件表示に切り替え</a> &ensp; ";
$change_row = $page_long * 2;
echo "<a href=\"./".basename(__FILE__)."?rawno=".$change_row."&page=".$page_no."&shuffle=".$is_shuffle."&img=".$on_img."&dirimg=".$dir_img."&path=".rawurlencode($path)."\">"."１ページ".$change_row."件表示に切り替え</a>\n";
echo"<br>\n";

//並び替え選択
if($is_shuffle == 0){
    echo "<a href=\"./".basename(__FILE__)."?rawno=".$page_long."&page=".$page_no."&shuffle="."1"."&img=".$on_img."&dirimg=".$dir_img."&path=".rawurlencode($path)."\">"."シャッフルする</a><br>\n";
}else{
    echo "<a href=\"./".basename(__FILE__)."?rawno=".$page_long."&page=".$page_no."&shuffle="."0"."&img=".$on_img."&dirimg=".$dir_img."&path=".rawurlencode($path)."\">"."通常の並びへ戻す</a><br>\n";
}

//ディレクトリ画像オンオフ
if($dir_img == 0){
    echo "<a href=\"./".basename(__FILE__)."?rawno=".$page_long."&page=".$page_no."&shuffle=".$is_shuffle."&img=".$on_img."&dirimg="."1"."&path=".rawurlencode($path)."\">"."ディレクトリ代表画像の表示</a><br>\n";
}else{
    echo "<a href=\"./".basename(__FILE__)."?rawno=".$page_long."&page=".$page_no."&shuffle=".$is_shuffle."&img=".$on_img."&dirimg="."0"."&path=".rawurlencode($path)."\">"."ディレクトリ代表画像の非表示</a><br>\n";
}

//画像オンオフ
if($on_img == 0){
    echo "<a href=\"./".basename(__FILE__)."?rawno=".$page_long."&page=".$page_no."&shuffle=".$is_shuffle."&img="."1"."&dirimg=".$dir_img."&path=".rawurlencode($path)."\">"."ディレクトリ直下の画像・音楽・ビデオも含める</a></p>\n";
}else{
    echo "<a href=\"./".basename(__FILE__)."?rawno=".$page_long."&page=".$page_no."&shuffle=".$is_shuffle."&img="."0"."&dirimg=".$dir_img."&path=".rawurlencode($path)."\">"."ディレクトリのみの表示に変更</a></p>\n";
}
...
if($page_no < $max_page){
    $next_page = $page_no + 1;
    echo "<a href=\"./".basename(__FILE__)."?rawno=".$page_long."&page=".$next_page."&shuffle=".$is_shuffle."&img=".$on_img."&dirimg=".$dir_img."&path=".rawurlencode($path)."\" class = \"next_btn\"> NEXT(".$next_page."ページ) &gt; </a>  <br>\n";
}
for($i = 1; $i <= $max_page; $i++){
    if($i == $page_no){ //現在のページはリンクを張らない
        echo "<b>{$page_no}</b>  ";
    }else{
        echo "<a href=\"./".basename(__FILE__)."?rawno=".$page_long."&page=".$i."&shuffle=".$is_shuffle."&img=".$on_img."&dirimg=".$dir_img."&path=".rawurlencode($path)."\">".$i."</a>  ";
    }
}
echo "</div>\n";
echo "<br>\n";
...
foreach($disp_list as $key => $folder){
    ...
    if($dir_img == 0){
        $key ++;
        echo "<a href = \"./imageshow.php?path=".rawurlencode($link)."\">".$key.": {$folder}</a>(画像一覧表示へ移動)";
        $key --;
    }else{
        if(($img = gettopimage($folder,3)) == NULL){
        //画像が存在しないときはディレクトリへのリンクを表示
        $key ++;
        echo "<a href = \"/".webPathEncode($link)."\">".$key.": ".htmlspecialchars($folder)."</a>（直近３階層ディレクトリ内画像なし）";
        $key --;
        }
        else{
            $key ++;
            ...
            echo "<br><a href = \"./imageshow.php?path=".rawurlencode($link)."\">".$key.": ".htmlspecialchars($folder)."</a>";
            $key --;
        }
    }
    echo "<br>";
